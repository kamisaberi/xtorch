...
namespace xt::transforms::image {
    ...
    auto OverSampling::forward(std::initializer_list<std::any> tensors) -> std::any {
        ...
        cv::Mat img_mat = tensor_to_mat_local(img_tensor);
        ...
        for (const auto& rect : crop_rects) {
            ...
            augmented_tensors.push_back(mat_to_tensor_local(cropped_mat));
            augmented_tensors.push_back(mat_to_tensor_local(flipped_mat));
        }
