...
namespace xt::optim
{
    ...
    void PLOOptions::serialize(torch::serialize::OutputArchive& archive) const
    {
        ...
        archive.write("k", k());
        archive.write("beta_momentum", beta_momentum());
        archive.write("beta_stats", beta_stats());
    ...
    torch::Tensor PLO::step(LossClosure closure)
    {
        ...
        for (auto& group : param_groups_)
        {
            for (auto& p : group.params())
            {
                ...
                {
                    state.l_inv_root = _compute_matrix_inverse_root(state.l_ema, group_options.damping(),
                                                                    group_options.root_order());
                    state.r_inv_root = _compute_matrix_inverse_root(state.r_ema, group_options.damping(),
                                                                    group_options.root_order());
                }
                ...
                auto preconditioned_grad = (p.dim() == 2)
                                               ? preconditioned_grad_reshaped
                                               : preconditioned_grad_reshaped.reshape(p.sizes());
                ...
                momentum_buffer.mul_(group_options.beta_momentum()).add_(
                    projected_update, 1.0 - group_options.beta_momentum());
    ...
    // --- Helper Functions ---
    void PLO::_fallback_to_sgd(torch::Tensor& param, const torch::Tensor& grad, PLOParamState& state,
    ...
    std::unique_ptr<torch::optim::OptimizerParamState> PLO::make_param_state()
    {
        return std::make_unique<PLOParamState>();
    }
}
