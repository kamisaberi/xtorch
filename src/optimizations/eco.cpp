...
namespace xt::optim
{
    ...
    torch::Tensor ECO::step(LossClosure closure)
    {
        ...
        for (auto& p : param_groups_[0].params())
        {
                ...
                static_cast<long>(current_step_val) % group_options.precondition_frequency() == 0)
            {
                ...
                state.inv_root_factors[0] = _compute_matrix_inverse_root(
                    state.fast_ema_factors[0], damping, group_options.root_order());
                state.inv_root_factors[1] = _compute_matrix_inverse_root(
                    state.fast_ema_factors[1], damping, group_options.root_order());
            }
    ...
    // --- Helper Functions ---
    void ECO::_fallback_to_adam(torch::Tensor& param, const torch::Tensor& grad, ECOParamState& state,
    ...
    std::unique_ptr<torch::optim::OptimizerParamState> ECO::make_param_state()
    {
        return std::make_unique<ECOParamState>();
    }
}
