...
struct AdaBeliefParamState : torch::optim::OptimizerParamState
{
    ...
    void deserialize(torch::serialize::InputArchive& archive)
    {
        ...
        {
            TORCH_WARN("Could not read 'step' for AdaBeliefParamState");
        ...
        {
            TORCH_WARN("Could not read 'exp_avg' for AdaBeliefParamState");
        ...
        {
            TORCH_WARN("Could not read 'exp_avg_var' for AdaBeliefParamState");
            if (!exp_avg_var_.defined()) exp_avg_var_ = torch::empty({0});
        }
    }
