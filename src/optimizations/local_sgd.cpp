#include "include/optimizations/local_sgd.h"

namespace xt::optimizations
{
    LocalSGD::LocalSGD(std::vector<torch::Tensor>&& parameters, double lr, double momentum)
    ...
    torch::Tensor LocalSGD::step(LossClosure closure)
    {
        return torch::Tensor();
    }
}
