...
torch::Tensor Lookahead::step(LossClosure closure) {
    ...
    auto& group_options = static_gcast<LookaheadOptions&>(param_groups_[0].options());
    ...
    for (auto& p : param_groups_[0].params()) {
        ...
        if (!state.step().defined()) {
            ...
            // 'slow_param' is initialized in the constructor
        }
        ...
        // This updates the "fast weights", which are the model's actual parameters `p`.
    ...
    // This happens for all parameters at once after they have all been updated.
