...
{
    StochasticWeightAveraging::StochasticWeightAveraging(std::vector<torch::Tensor>&& parameters, double lr, double momentum)
    ...
    torch::Tensor StochasticWeightAveraging::step(LossClosure closure)
    {
        return torch::Tensor();
    }
}
