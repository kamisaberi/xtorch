...
torch::Tensor FATA::step(LossClosure closure) {
    ...
    for (auto& p : param_groups_[0].params()) {
        ...
        if (state.fast_ema_factors.empty()) {
            ...
            state.fast_ema_factors[0] = torch::zeros_like(L_stat); state.fast_ema_factors[1] = torch::zeros_like(R_stat);
            state.slow_ema_factors[0] = torch::zeros_like(L_stat); state.slow_ema_factors[1] = torch::zeros_like(R_stat);
        ...
        auto preconditioned_grad = (p.dim() == 2) ? preconditioned_grad_reshaped.t() : preconditioned_grad_reshaped.t().reshape(p.sizes());
...
// --- Helper Functions ---
void FATA::_fallback_to_adam(torch::Tensor& param, const torch::Tensor& grad, FATAParamState& state, const FATAOptions& options) {
    auto& m = state.exp_avg();
    auto& v = state.exp_avg_sq();
...
std::unique_ptr<torch::optim::OptimizerParamState> FATA::make_param_state() { return std::make_unique<FATAParamState>(); }