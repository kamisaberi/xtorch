...
bool download_file(const std::string& url, const fs::path& destination_path) {
    ...
    if (result == 0 && fs::exists(destination_path)) {
        ...
        if (fs::exists(destination_path)) { // Clean up partial download
...
bool convert_hf_model_to_torchscript_from_lib(
    ...
    if (!fs::exists(paths.conversion_script)) {
        std::cerr << "xTorch Utils ERROR: Python conversion script not found at: " << paths.conversion_script << std::endl;
        return false;
    }
    ...
    if (result == 0) {
        std::cout << "[xTorch Utils] Python model conversion script executed successfully (or reported success)." << std::endl;
        return true;
    } else {
        std::cerr << "[xTorch Utils] Python model conversion script failed or reported error. Exit code: " << result << std::endl;
        return false;
    }
}
