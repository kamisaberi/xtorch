...
namespace xt::losses
{
     ...
     * @return Scalar tensor containing the Hinge Loss
     */
    torch::Tensor gan_hinge_loss(const torch::Tensor& d_real_logits, const torch::Tensor& d_fake_logits,
                                 const std::string& mode)
    {
        ...
        if (mode == "discriminator")
        {
            ...
            torch::Tensor real_loss = torch::relu(1.0 - d_real_logits);
            torch::Tensor fake_loss = torch::relu(1.0 + d_fake_logits);
    ...
    {
        return xt::losses::gan_hinge_loss(torch::zeros(10), torch::zeros(10), "generator");
    }
}
