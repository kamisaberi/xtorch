...
namespace xt::losses
{
     ...
     */
    torch::Tensor ghmc_loss(const torch::Tensor& logits, const torch::Tensor& labels, int bins = 10,
                            float momentum = 0.9,
                            float eps = 1e-6)
    {
    ...
    {
        return xt::losses::ghmc_loss(torch::zeros(10),torch::zeros(10));
    }
}
