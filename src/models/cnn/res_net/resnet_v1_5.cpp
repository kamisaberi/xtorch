#include "../../../../include/models/cnn/resnet/resnet-v1.5.h"
...
namespace xt::models {
    ...
    ResNetV15::ResNetV15(vector<int> layers, int num_classes, int in_channels) : BaseModel() {
        inplanes = 64;
    ...
    ResNetV15::ResNetV15(std::vector<int> layers, int num_classes, int in_channels, std::vector<int64_t> input_shape)
        : BaseModel() {
        inplanes = 64;
    ...
    torch::nn::Sequential ResNetV15::makeLayerFromResidualBlock(int planes, int blocks, int stride) {
        torch::nn::Sequential downsample = nullptr;
        if (stride != 1 || inplanes != planes) {
            downsample = torch::nn::Sequential();
    ...
    torch::Tensor ResNetV15::forward(torch::Tensor x) const {
        x = conv1->forward(x);
        x = maxpool->forward(x);
        x = layer0->forward(x);
        x = layer1->forward(x);
