#include "models/computer_vision/image_classification/resnext.h"
...
namespace xt::models {
    ...
    torch::nn::Sequential ResNext::makeLayerFromResidualBlock(int planes, int blocks, int stride) {
        torch::nn::Sequential downsample = nullptr;
        if (stride != 1 || inplanes != planes) {
            downsample = torch::nn::Sequential();
    ...
    torch::Tensor ResNext::forward(torch::Tensor x) const {
        x = conv1->forward(x);
        x = maxpool->forward(x);
        x = layer0->forward(x);
        x = layer1->forward(x);
