...
struct BigBirdAttention : torch::nn::Module {
    int num_heads;
    int head_size;
    int block_size;
    int num_rand_blocks;
    ...
    BigBirdAttention(int hidden_size, int num_heads, int block_size, int num_rand_blocks, int num_global_tokens, float dropout_p = 0.1)
            : num_heads(num_heads), head_size(hidden_size / num_heads), block_size(block_size),
              num_rand_blocks(num_rand_blocks), num_global_tokens(num_global_tokens) {
    ...
    torch::Tensor forward(torch::Tensor x, torch::Tensor attention_mask = {}) {
        ...
        auto sparse_mask = create_sparse_attention_mask(batch_size, seq_len);
...
private:
    torch::Tensor create_sparse_attention_mask(int batch_size, int seq_len) {
        auto mask = torch::zeros({batch_size, num_heads, seq_len, seq_len}, torch::kFloat32);
        std::random_device rd;
        std::mt19937 gen(rd());

        // Global attention for first num_global_tokens (e.g., [CLS])
        for (int i = 0; i < num_global_tokens; ++i) {
            mask.slice(2, i, i + 1).fill_(1.0); // Attend to all
            mask.slice(3, i, i + 1).fill_(1.0); // All attend to global tokens
        }

        // Sliding window attention
        int window_size = block_size * 3; // 3 blocks as per BigBird
        ...
        // Random attention
        std::uniform_int_distribution<int> dist(0, seq_len - 1);
        for (int i = 0; i < seq_len; ++i) {
            for (int j = 0; j < num_rand_blocks; ++j) {
                int rand_pos = dist(gen);
                mask.slice(2, i, i + 1).slice(3, rand_pos, rand_pos + 1).fill_(1.0);
            }
        }
...
struct TransformerLayer : torch::nn::Module {
    BigBirdAttention attention{nullptr};
    torch::nn::LayerNorm norm1{nullptr}, norm2{nullptr};
    FeedForward ffn{nullptr};
    torch::nn::Dropout dropout{nullptr};

    TransformerLayer(int hidden_size, int num_heads, int intermediate_size, int block_size, int num_rand_blocks, int num_global_tokens, float dropout_p = 0.1)
            : attention(BigBirdAttention(hidden_size, num_heads, block_size, num_rand_blocks, num_global_tokens, dropout_p)),
...
// BigBird Model
struct BigBird : torch::nn::Module {
    TransformerEmbedding embedding{nullptr};
    torch::nn::ModuleList layers{nullptr};
    torch::nn::Linear mlm_head{nullptr};

    BigBird(int vocab_size, int hidden_size, int num_heads, int intermediate_size, int num_layers, int max_position, int block_size, int num_rand_blocks, int num_global_tokens) {
        ...
        for (int i = 0; i < num_layers; ++i) {
            layers->push_back(TransformerLayer(hidden_size, num_heads, intermediate_size, block_size, num_rand_blocks, num_global_tokens));
        }
...
struct MLMDataset {
    ...
    Vocabulary vocab;
    const int max_len = 512; // Longer sequences for BigBird
...
int main() {
    ...
    // Initialize model
    BigBird model(dataset.vocab.vocab_size, 256, 4, 1024, 6, 512, 64, 2, 1);
    ...
    // Save model
    torch::save(model, "bigbird_model.pt");
