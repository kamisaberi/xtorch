...
// Custom Prompt-Response Dataset with MTP
class PromptResponseDataset : public torch::data::Dataset<PromptResponseDataset> {
public:
    PromptResponseDataset(const std::string& prompt_dir, const std::string& response_dir, const Vocabulary& vocab, int max_seq_length, int mtp_size = 2)
        : vocab_(vocab), max_seq_length_(max_seq_length), mtp_size_(mtp_size) {
        for (const auto& entry : std::filesystem::directory_iterator(prompt_dir)) {
            if (entry.path().extension() == ".txt") {
                prompt_paths_.push_back(entry.path().string());
                std::string response_path = response_dir + "/" + entry.path().filename().string();
    ...
    torch::data::Example<> get(size_t index) override {
        ...
        // Create MTP targets (predict next mtp_size_ tokens)
        std::vector<torch::Tensor> target_tensors;
        for (int i = 1; i <= mtp_size_; ++i) {
            std::vector<int64_t> target_indices(token_indices.begin() + i, token_indices.end());
            target_indices.resize(max_seq_length_ - i, vocab_.token_to_idx("<pad>"));
            target_tensors.push_back(torch::tensor(target_indices, torch::kInt64));
        }
        torch::Tensor target_tensor = torch::stack(target_tensors, 0); // [mtp_size, seq_len]
...
// MoE Layer with Sigmoid Routing
struct MoELayerImpl : torch::nn::Module {
    MoELayerImpl(int d_model, int dim_feedforward, int num_experts, int top_k) : top_k_(top_k) {
        gating = register_module("gating", torch::nn::Linear(d_model, num_experts));
        for (int i = 0; i < num_experts; ++i) {
    ...
    torch::Tensor forward(torch::Tensor x) {
        auto gate_logits = gating->forward(x); // [batch, seq_len, num_experts]
        auto weights = torch::sigmoid(gate_logits); // [batch, seq_len, num_experts]
        auto topk = torch::topk(weights, top_k_, /*dim*/ -1);
        auto top_weights = std::get<0>(topk); // [batch, seq_len, top_k]
        auto indices = std::get<1>(topk); // [batch, seq_len, top_k]
        top_weights = top_weights / top_weights.sum(-1, true); // Normalize

        torch::Tensor output = torch::zeros_like(x);
        for (int i = 0; i < top_k_; ++i) {
            auto w = top_weights.select(-1, i).unsqueeze(-1); // [batch, seq_len, 1]
            auto idx = indices.select(-1, i); // [batch, seq_len]
            torch::Tensor expert_output = torch::zeros_like(x);
            for (int b = 0; b < x.size(0); ++b) {
                for (int s = 0; s < x.size(1); ++s) {
...
// Simplified DeepSeek-V3-like Model
struct DeepSeekV3Impl : torch::nn::Module {
    DeepSeekV3Impl(int vocab_size, int d_model = 256, int nhead = 4, int num_layers = 1, int dim_feedforward = 512, int num_experts = 16, int top_k = 8, int mtp_size = 1, float dropout = 0.1) : mtp_size_(mtp_size) {
        token_embedding = torch::nn::Embedding(vocab_size, d_model);
        register_module("token_embedding", token_embedding);
        positional_encoding = torch::randn({1, 128, d_model});
        register_parameter("positional_encoding", positional_encoding);
        encoder = TransformerEncoderImpl(d_model, nhead, num_layers, dim_feedforward, num_experts, top_k, dropout);
        register_module("encoder", encoder);
        output_layers = torch::nn::ModuleList();
        for(int i = 0; i < mtp_size; ++i) {
            auto linear = torch::nn::Linear(d_model, vocab_size);
            output_layers->push_back(linear);
            register_module("output_layer_" + std::to_string(i), linear);
        }
    }
    ...
    std::vector<torch::Tensor> forward(torch::Tensor input) {
        auto features = token_embedding->forward(input); // [batch, seq_len, d_model]
        features = features + positional_encoding.slice(1, 0, input.size(1));
        auto top_k_mask = torch::triu(torch::ones({input.size(1), input.size(1)}), 1).to(torch::kBool);
        auto encoded = encoder->forward(features, top_k_mask); // [batch, seq_len, d_model]
        std::vector<torch::Tensor> outputs;
        for (int i = 0; i < mtp_size_; ++i) {
            auto output_layer = output_layers[i]->as<torch::nn::Linear>();
            outputs.push_back(output_layer->forward(encoded)); // [batch, seq_length, vocab_size]
        }
        return outputs;
    }
...
TORCH_MODULE(DeepSeekV3);

// Training function
void train_step(DeepSeekV3 model, torch::optim::Optimizer optimizer, torch::Tensor inputs, torch::vector<torch::Tensor> targets, torch::Device device, int vocab_size, int mtp_size, float& total_loss, int& batch_count) {
    inputs = inputs.to(device);
    std::vector<torch::Tensor> target_tensors;
    for (int i = 0; i < mtp_size; ++i) {
        target_tensors.push_back(targets[i].to(device));
    }

    optimizer->zero_grad();
    auto outputs = model->forward(inputs); // List of [batch, seq_len, vocab_size]
    torch::Tensor loss = torch::zeros({}).to(device);
    for(int i = 0; i < mtp_size; ++i) {
        loss += torch::nn::functional::cross_entropy(
            outputs[i].view({-1, vocab_size}),
            target_tensors[i].view({-1}));
    }
    loss = loss / mtp_size; // Average loss across MTP predictions
    loss.backward();
    optimizer->step();
...
int main() {
    try {
        ...
        // Initialize model
        DeepSeekV3 model(vocab.vocab_size(), d_model, nhead, num_layers, dim_feedforward, num_experts, top_k, mtp_size, dropout);
        model->to(device);
        ...
        // Dataset and DataLoader
        auto dataset = PromptResponseDataset("./data/prompts", "./data/responses", vocab, max_seq_length, mtp_size)
            .map(torch::data::transforms::Stack<>());
        auto data_loader = torch::data::make_data_loader(
            std::move(dataset),
            torch::data::DataLoaderOptions().batch_size(batch_size).workers(2));
        ...
        for (int epoch = 0; epoch < num_epochs; ++epoch) {
            ...
            for (auto& batch : *data_loader) {
                ...
                auto targets = batch.target; // [mtp_size, batch, seq_len]
                std::vector<torch::Tensor> target_tensors;
                for (int i = 0; i < mtp_size; ++i) {
                    target_tensors.push_back(targets[i]);
                }
                train_step(model, optimizer, inputs, target_tensors, device, vocab.vocab_size(), mtp_size, total_loss, batch_count);
            }
            ...
            // Save checkpoint every 5 epochs
            if ((epoch + 1) % 5 == 0) {
                torch::save(model, "deepseek_v3_epoch_" + std::to_string(epoch + 1) + ".pt");
                std::cout << "Saved checkpoint: deepseek_v3_epoch_" << epoch + 1 << ".pt" << std::endl;
            }
        }

        // Save final model
        torch::save(model, "deepseek_v3_final.pt");
        std::cout << "Saved final model: deepseek_v3_final.pt" << std::endl;
