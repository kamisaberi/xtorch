#include <models/generative_models/gans/pro_gan.h>
...
namespace xt::models
{
    ...
    ProGAN::ProGANGeneratorImpl::ProGANGeneratorImpl(int latent_dim) : latent_dim_(latent_dim)
    {
        initial = register_module("initial", torch::nn::Linear(latent_dim, 256 * 4 * 4));
        block1 = register_module("block1", GeneratorBlock(256, 128)); // 4x4 -> 8x8
        block2 = register_module("block2", GeneratorBlock(128, 64)); // 8x8 -> 16x16
    ...
    torch::Tensor ProGAN::ProGANGeneratorImpl::forward(torch::Tensor z, int resolution_level, float alpha)
    {
        auto batch_size = z.size(0);
        z = relu->forward(initial->forward(z)); // [batch, 256 * 4 * 4]
        z = z.view({batch_size, 256, 4, 4}); // [batch, 256, 4, 4]
    ...
    torch::Tensor ProGAN::DiscriminatorBlockImpl::forward(torch::Tensor x)
    {
        ...
        return x;
                ...
                ProGAN::ProGANDiscriminatorImpl::ProGANDiscriminatorImpl()
            {
                from_img_28x28 = register_module("from_img_28x28", torch::nn::Conv2d(
                                                     torch::nn::Conv2dOptions(1, 32, 3).stride(1).padding(1)));
                from_img_16x16 = register_module("from_img_16x16", torch::nn::Conv2d(
            ...
            torch::Tensor ProGAN::ProGANDiscriminatorImpl::forward(torch::Tensor x, int resolution_level, float alpha)
            {
                auto batch_size = x.size(0);
