#include <models/generative_models/gans/style_gan.h>
...
namespace xt::models
{
    ...
    torch::Tensor StyleGAN::MappingNetwork::forward(torch::Tensor z)
    {
        auto x = torch::leaky_relu(fc0->forward(z), 0.2);
        x = torch::leaky_relu(fc1->forward(x), 0.2);
        x = torch::leaky_relu(fc2->forward(x), 0.2);
    ...
    torch::Tensor StyleGAN::SynthesisBlock::forward(torch::Tensor x, torch::Tensor w)
    {
        x = conv->forward(x);
        auto ys = style_s->forward(w);
        auto yb = style_b->forward(w);
    ...
    torch::Tensor StyleGAN::SynthesisNetwork::forward(torch::Tensor w)
    {
        auto x = initial->forward(w).view({-1, 64, 4, 4});
        x = block1->forward(x, w);
        x = torch::upsample_nearest2d(x, {8, 8});
    ...
    torch::Tensor StyleGAN::StyleGANGenerator::forward(torch::Tensor z)
    {
        auto w = mapping->forward(z);
        return synthesis->forward(w);
    }
