#include <models/generative_models/gans/big_gan.h>
...
namespace xt::models
{
    ConditionalBatchNorm::ConditionalBatchNorm(int64_t num_features, int64_t num_classes)
        : bn(num_features),
          gamma_embed(torch::nn::Linear(num_classes, num_features)),
          beta_embed(torch::nn::Linear(num_classes, num_features))
    {
    ...
    torch::Tensor ConditionalBatchNorm::forward(torch::Tensor x, torch::Tensor y)
    {
        auto out = bn->forward(x);
        auto gamma = gamma_embed->forward(y).unsqueeze(2).unsqueeze(3);
        auto beta = beta_embed->forward(y).unsqueeze(2).unsqueeze(3);
    ...
    SpectralNormConv::SpectralNormConv(torch::nn::Conv2dOptions options)
        : conv(options)
    {
        register_module("conv", conv);
        u = register_buffer("u", torch::randn({1, options.out_channels()}).normal_(0, 1));
    ...
    torch::Tensor SpectralNormConv::forward(torch::Tensor x)
    {
        // Power iteration for spectral norm
        auto weight = conv->weight.view({conv->weight.size(0), -1});
        auto v = torch::matmul(weight.t(), u.t()).t();
    ...
    BigGAN::GeneratorBlock::GeneratorBlock(int64_t in_channels, int64_t out_channels, int64_t num_classes)
    {
    ...
    torch::Tensor BigGAN::GeneratorBlock::forward(torch::Tensor x, torch::Tensor y)
    {
    ...
    BigGAN::BigGAN(int64_t z_dim, int64_t num_classes): linear(torch::nn::Linear(z_dim + num_classes, 4 * 4 * 256))
    {
    ...
    auto BigGAN::forward(std::initializer_list<std::any> tensors) -> std::any
    {
        ...
        torch::Tensor z = tensor_vec[0];
        torch::Tensor y = tensor_vec[1];
