...
//         auto max_scores = scores.max(-1).values.unsqueeze(-1); // [batch, n_heads, seq_len, 1]
//         auto sparsity_mask = torch::logsumexp(scores - max_scores, -1) - max_scores.squeeze(-1); // [batch, n_heads, seq_len]
//         auto topk = sparsity_mask.topk(factor * static_cast<int>(std::log(seq_len)), -1, true, true);
...
//         auto out = torch::bmm(attn, v); // [batch, n_heads, seq_len, head_dim]
//         out = out.transpose(1, 2).contiguous().view({batch_size, seq_len, d_model});
//         return out_linear->forward(out);
//     }
// };
//
// // Transformer Encoder Layer with Distilling
...
//
//         // Distilling: halve sequence length
//         x = x.transpose(1, 2); // [batch, d_model, seq_len]
//         x = distil_conv->forward(x); // [batch, d_model, seq_len/2]
...
//
// // Informer Model
// struct Informer : torch::nn::Module {
...
//
//     Informer(int seq_len_, int pred_len_, int d_model_, int n_heads, int e_layers, int d_layers, int d_ff, int factor, int max_tokens = 1000)
...
//         // Encoder
//         auto src_emb = input_projection->forward(src); // [batch, seq_len, d_model]
...
//         auto tgt_emb = token_embedding->forward(tgt_tokens); // [batch, pred_len, d_model]
...
//
//         // Output projection
//         return output_projection->forward(dec_out); // [batch, pred_len, 1]
//     }
// };
//
// // Time-Series Dataset
...
//
//     // Load dataset
//     TimeSeriesDataset dataset("timeseries_data.csv", 96, 48); // seq_len=96, pred_len=48
...
//
//             optimizer.zero_grad();
//             auto output = model.forward(src, tgt); // [batch, pred_len, 1]
...
//
//     // Save model
//     torch::save(model, "informer_model.pt");
//
//     return 0;
// }
