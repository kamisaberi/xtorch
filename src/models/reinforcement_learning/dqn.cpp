#include "include/models/reinforcement_learning/dqn.h"
...
public:
    ...
    DQNAgent(int state_dim_, int action_dim_, float gamma_, float epsilon_, float epsilon_min_,
    ...
    void train_step(int batch_size, std::mt19937& gen) {
        ...
        // DQN: Use target net for next Q-values
        torch::NoGradGuard no_grad;
...
int main() {
    ...
    // Save policy network
    torch::save(agent.policy_net, "dqn_policy.pt");
...
namespace xt::models
{
    DQN::DQN(int num_classes, int in_channels)
    {
    }

    DQN::DQN(int num_classes, int in_channels, std::vector<int64_t> input_shape)
    {
    }
    ...
    auto DQN::forward(std::initializer_list<std::any> tensors) -> std::any
    {
