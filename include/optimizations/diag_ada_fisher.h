...
struct DiagonalAdaFisherParamState : torch::optim::OptimizerParamState {
    ...
    void deserialize(torch::serialize::InputArchive& archive)  {
        ...
        else { TORCH_WARN("Could not read 'step' for DiagonalAdaFisherParamState"); if(!step_.defined()) step_ = torch::empty({0});}

        if (archive.try_read("fisher_diag_ema", temp_tensor, true)) { fisher_diag_ema_ = temp_tensor; }
        else { TORCH_WARN("Could not read 'fisher_diag_ema' for DiagonalAdaFisherParamState"); if(!fisher_diag_ema_.defined()) fisher_diag_ema_ = torch::empty({0});}
    }
