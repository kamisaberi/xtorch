...
namespace xt {
                      ...
                      torch::Device device)
    {
        ...
        for (current_epoch_ = 1; current_epoch_ <= max_epochs_; ++current_epoch_)
        {
            ...
            if (lr_scheduler_)
            {
                if (auto* plateau_scheduler = dynamic_cast<torch::optim::ReduceLROnPlateauScheduler*>(lr_scheduler_.get()))
                {
    ...
    void train_epoch(LoaderT& train_loader, torch::Device device) // <<< FIX WAS HERE
    {
        ...
        for (auto& batch_data : train_loader)
        {
            ...
            BatchEndState batch_state(current_epoch_, static_cast<int>(batch_idx), loss.detach() * grad_accumulation_steps_, output.detach(), target.detach(), data.size(0));
