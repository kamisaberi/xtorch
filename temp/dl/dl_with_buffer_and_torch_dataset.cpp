...
public:
    MyCustomDataLoaderV2(MyTorchDataset& dataset,
        ...
        : dataset_(dataset),
          ...
          prefetch_queue_max_size_(std::max(size_t(1), num_workers * prefetch_factor)), // Ensure queue size is at least 1
    ...
    std::optional<BatchData> next_batch() {
        if (current_dataset_size_ == 0) { // Handle empty dataset upfront
            return std::nullopt;
        }
...
private:
        ...
        try {
            while (true) {
                ...
                if (current_batch_overall_idx >= total_batches_in_epoch_) {
                    ...
                    if (!epoch_ended_for_workers_.load()) { // New epoch has started
    ...
    std::optional<BatchData> produce_batch(size_t batch_overall_idx) {
        ...
        if (actual_batch_size == 0) { // Can happen if dataset_size is 0 or for last batch if miscalculated
            return std::nullopt;
        }
        ...
        if (batch_features_list.empty()) { // Should only happen if actual_batch_size was 0 initially
            return std::nullopt;
        }
...
public: // Iterator support
    ...
    public:
        ...
        const BatchData& operator*() const {
         ...
         BatchData& operator*() { // Non-const version
            if (!current_batch_opt_) {
                 throw std::runtime_error("Attempting to dereference an end iterator or uninitialized iterator.");
            }
            return *current_batch_opt_;
        ...
        Iterator& operator++() {
            if (loader_ && !is_end_) { // Only advance if not already at the end
                current_batch_opt_ = loader_->next_batch();
                if (!current_batch_opt_) {
    ...
    Iterator begin() {
        ...
        if (current_dataset_size_ == 0 || total_batches_in_epoch_ == 0) { // Handle empty dataset immediately
...
int main() {
    ...
    for (int epoch = 1; epoch <= num_epochs; ++epoch) {
        ...
        for (const auto& batch : data_loader) { // data_loader.begin() calls reset_epoch()
    ...
    for (const auto& batch : empty_loader) { // This loop should not execute
        empty_batch_count++;
    ...
    std::cout << "\nMain function finished. DataLoader objects will now be destroyed, shutting down workers." << std::endl;
    return 0;
}