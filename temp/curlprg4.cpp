...
public:
    SpinnerBar(int w = 50) : width(w), start(steady_clock::now()) {}
    ...
    void update(curl_off_t total, curl_off_t now) {
        ...
        const string color = "\033[38;5;197m"; // Pink-red
        ...
        cout << "] "
             << setw(5) << fixed << setprecision(1) << (ratio * 100) << "% "
             << setw(6) << setprecision(0) << speed << " KB/s "
             << "ETA: " << setw(3) << eta << "s"
             << reset << flush;

        this_thread::sleep_for(milliseconds(80));  // Smooth spinner
    }
};
